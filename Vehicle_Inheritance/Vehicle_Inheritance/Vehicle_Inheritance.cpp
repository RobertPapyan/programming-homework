...
ostream& operator<<(ostream& output, Vehicle& v1) {
	...
	output << "Stats: " << "Manufactor " << v1.getManufactor() << ", Year " << v1.getManufactorYear() << ", Price " << v1.getPrice() << ", Weight in Tonnes " << v1.getWeight() << ", Max speed " << v1.getMaxVelocity() << endl;
	return output;
}
...
ostream& operator<<(ostream& output, Car& v1) {
	output << static_cast<Vehicle&>(v1) << ", Seat count " << v1.getSeatCount() << endl;
	/*
...
ostream& operator<<(ostream& output, Truck& v1) {
	...
	output << "Engine: " << v1.engine.engineModel << ' ' << v1.engine.enginePower << ' ' << Engine::typeToString(v1.engine.getEngineType()) << endl;
	output << "Stats: " << "Manufactor " << v1.getManufactor() << ", Year " << v1.getManufactorYear() << ", Price " << v1.getPrice()
		<< ", Weight in Tonnes " << v1.getWeight() << ", Max speed " << v1.getMaxVelocity() << ", Capacity " << v1.getCapacity() << endl;
	return output;
}
...
int main()
{
	...
	*/
	...
	Vehicle* testVehicle = new Car(Engine("V-4", 180, 2.0, Types::petrol), "Mercedes", "C", 25000, 2.5, 220, 2005,5);
	...
	vehicleList[0]->run(67);
	vehicleList[0]->run(0);
	vehicleList[2]->run(50);
